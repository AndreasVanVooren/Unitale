...
function PlaySeparate()
	...
	sepGradient.sprite.xscaleDim = 1;
	sepGradient.sprite.yscaleDim = 1;
...
function CoreUpdateLoop()
	...
	if(locketPlayed == true) then
		sepGradient.sprite.xscaleDim = (9 + math.max(0,(eventTimer - playLocketConst)))*0.0625;
		sepGradient.sprite.alpha = 1 - (eventTimer - playLocketConst)/ (playLocketConst/2)
	else
		sepGradient.sprite.xscaleDim = (9 + math.sin(eventTimer))*0.0625;
